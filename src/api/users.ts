...
const getUsers = async (logedInUser: User) => {
    const q = query(collection(db, "users"), where("uid", "!=", logedInUser.uid))
    ...
    const users = querySnapshot.docs.map(doc => {
        ...
        const hasFollowing = logedInUser.following.filter(following => following.id === user.uid).length > 0
        return !hasFollowing
    })
    return users;
