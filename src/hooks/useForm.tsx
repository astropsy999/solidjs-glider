...
const niceName = (text: string) => {
  ...
  export const requiredValidfator: Validator =
  ...
  export const maxLengthValidator: Validator =
    (element: HTMLInputElement, maxLength = 10) =>
    (form: Form) => {
  ...
  export const upperCaseLetterValidator: Validator =
    (element: HTMLInputElement) => (form: Form) => {
      const { value } = element;
    ...
    export const compareWith: Validator = (element: HTMLInputElement, fieldName: string ) => (form: Form) => {
      ...
      return element.value !== compareToValue ? `${niceName(element.name)} should be same as ${niceName(fieldName)}` : ""
    }
...
const useForm = <T extends Form>(initialForm: T) => {
     ...
     const submitForm = (submitCallback: SubmitCallback<T>) => () => {
        ...
        if (isValid()) {submitCallback(form)};
     };
